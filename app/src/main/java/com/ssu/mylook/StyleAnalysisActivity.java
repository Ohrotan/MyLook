...
//                        Log.d("TAG", task.getResult().size() + "asdfghj");
        ...
        db.collection("clothes")
                ...
                .addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
                    @Override
                    public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                        ClotheDTO dto = queryDocumentSnapshots.getDocuments().get(0).toObject(ClotheDTO.class);
                        new DBUtil().setImageViewFromDB(StyleAnalysisActivity.this, mostClothesImg, dto.getImage());
                        mostClothesName.setText(dto.getTitle());
                    }
                });
